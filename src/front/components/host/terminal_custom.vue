...
export default {
  ...
  methods: {
    ...
    writePrifix(lineNum) {
      ...
      if (this.contentHeight - this.fontSize > this.terminalHeight) { // 内容超过盒子大小，显示最下面的一行
        ...
        console.log('this.textY', this.textY)
  ...
  mounted() {
    ...
    this.$nextTick(() => {
      ...
      this.$nextTick(() => {
        ...
        // 初始化存储所有的文本数据的对象
        this.allText = this.$data.textCtx.createImageData(this.terminalWidth, this.terminalHeight)
  ...
  watch: {
    ...
    terminalHeight(val, old) {
      ...
      if (old !== 0) {
        if (old < val) {
          console.log(this.$data.contentHeight - this.$data.fontSize, this.$data.terminalHeight)
          if (this.$data.contentHeight - this.$data.fontSize < this.$data.terminalHeight) {
            ...
            console.log('>')
            this.textY = val - this.contentHeight + this.fontSize
